...
def classify_signature(request):
    if request.method == 'POST' and request.FILES.get('image'):
        ...
        # resized_img = resize_image(normalized_img, (250, 520))  # Resize the image
        cropped = crop_center(normalized_img, (250, 520))
        # Convert NumPy array back to PIL Image
        resized_img_pil = Image.fromarray((cropped * 255).astype(np.uint8))
